...
class CarsDataset(Dataset):
    ...
    def __init__(self, train=True, limit=0, root=None, transform=None):
        ...
        if train:
            data_dir = root_path / 'cars_train'
            metas = root_path / 'cars_train_annos.mat'
        else:
            data_dir = root_path / 'cars_test'
            metas = root_path / 'cars_test_annos_withlabels.mat'
...
def get_scars_datasets(root, train_transform, test_transform, train_classes=range(160), prop_train_labels=0.8,
                    split_train_val=False, seed=0):

    np.random.seed(seed)

    # Init entire training set
    whole_training_set = CarsDataset(root=root, transform=train_transform, train=True)
    ...
    test_dataset = CarsDataset(root=root, transform=test_transform, train=False)
